...
- Lisp, ~car~, ~cdr~, and ~cons~ are fundamental functions.  The
...
- In the walk through of the ~copy-region-as-kill~ function, we will
  see ~const~ as well as two variants on ~cdr~, namely, ~setcdr~ and
  ~nthcdr~.  (See [[][Copy-region-as-kill]].)

  - [[Strange Name][Strange Names]]       A historical aside: why the strange name?

  - [[~car~ and ~cdr~][car & cdr]]           A function for extracting part of a list.

  - [[~cons~][cons]]                Constructing a list.

  - [[~nthcdr][nthcdr]]              Calling ~cdr~ repeatedly.
...
- This means that the function receives the wrong number of arguments,
  ...
  function is measuring.  (Note that /one/ list is /one/ argument,
  even if the list has many elements inside it.)
